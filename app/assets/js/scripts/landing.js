...
const refreshMojangStatuses = async function () {
    loggerLanding.log('Rechargement des status mojang ...')
...
const refreshServerStatus = async function (fade = false) {
    loggerLanding.log('Status du serveur minecraft')
    let status = document.getElementById('player_count');
    ...
    try {
        ...
        const servStat = await getServerStatus(47, "game14-fr.hosterfy.com", Number("60004"))
        console.log(servStat)
        pLabel = 'Serveur : '
        pVal = "Connecté"
        
        status.style.color = "green"
        status.innerHTML = pVal 

    } catch (err) {
        loggerLanding.warn('Impossible de refresh le status.')
    ...
    sysAEx.on('message', (m) => {
        ...
        if (m.context === 'validateJava') {
            if (m.result == null) {
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            'Vous avez besoin de Java<br>pour lancer Minecraft',
                            'Une installation x64 valide de Java 8 est requise pour le lancement.<br><br>Nous pouvons le faire pour vous.',
                            'Installer Java',
                            'Non merci'
                        )
                        setOverlayHandler(() => {
            ...
            if (m.result === true) {
                ...
                setOverlayContent(
                    'Unexpected Issue:<br>Échec du téléchargement de Java',
                    'Malheureusement, nous avons rencontré un problème lors de la tentative d\'installation de Java. Vous devrez installer manuellement Java.',
                    'Je comprends'
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
        ...
        if (m.context === 'validate') {
                ...
                case 'download':
                    ...
                    if (m.error.code === 'ENOENT') {
                        showLaunchFailure(
                            ...
                            'Impossible de se connecter au serveur. Assurez-vous que vous êtes connecté à internet et réessayez.'
                        )
                    } else {
                        showLaunchFailure(
                            'Erreur téléchargement',
                            'Ouvrez un ticket sur le discord pour : Problème Launcher'
                        )
                    }
            ...
            if (login && allGood) {
                ...
                try {
                    ...
                    setLaunchDetails("Téléportation ...")
...
function displayArticle(articleObject, index) {
    ...
    newsArticleContentScrollable.innerHTML = '<div id="newsArticleContentWrapper" style="font-size: 1.30em;"><div class="newsArticleSpacerTop"></div>' + articleObject.content + '<div class="newsArticleSpacerBot"></div></div>'
