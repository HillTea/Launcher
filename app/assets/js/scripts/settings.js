        ...
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    ...
                    'Pour pouvoir utiliser le launcher il vous faut au minimum un compte. Vous aurez besoin de vous connecter après ça.<br><br>Souhaitez-vous vraiment vous déconnecter?',
                    'Je suis sûre',
                    'Annuler'
                )
                setOverlayHandler(() => {
    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
            ...
            setOverlayContent(
                ...
                "Microsoft, veuillez vous reconnecter s'il vous plaît.",
                'Ok'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
...
function refreshAuthAccountSelected(uuid){
    Array.from(document.getElementsByClassName('settingsAuthAccount')).map((val) => {
        ...
        if(uuid === val.getAttribute('uuid')){
            ...
            selBtn.innerHTML = 'Selectionnez un compte &#10004;'
        } else {
            if(selBtn.hasAttribute('selected')){
                selBtn.removeAttribute('selected')
            }
            selBtn.innerHTML = "Selection d'un compte"
        }
    })
}
        ...
        const accHtml = `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            ...
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Compte sélectionné &#10004;' : '>Compte choisi'}</button>
        ...
        <div class="serverListingDetails">
            ...
            <div class="serverListingInfo">
                ...
                ${serv.isMainServer() ? `<div class="serverListingStarWrapper">
                    ...
                    <span class="serverListingStarTooltip">Serveur</span>
                </div>` : ''}
            </div>
        </div>
    `
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            ...
            if(v.version.major < 9) {
                settingsJavaExecDetails.innerHTML = `Selectionnez la version java ${v.version.major} Update ${v.version.update} (x${v.arch})${vendor}`
            } else {
                settingsJavaExecDetails.innerHTML = `Selectionnez la version java ${v.version.major}.${v.version.minor}.${v.version.revision} (x${v.arch})${vendor}`
            }
        } else {
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/HillTea/Launcher/releases.atom',
        success: (data) => {
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Téléchargez depuis Github<span style="font-size: 10px;color: gray;text-shadow: none !important;">Fermez le launcher et cliquez sur le .dmg pour le lancer à nouveau.</span>', false, () => {
        ...
        settingsUpdateButtonStatus('Recherche des mises à jours ?', false, () => {
            if(!isDev){
                ipcRenderer.send('autoUpdateAction', 'checkForUpdate')
                settingsUpdateButtonStatus('Chargement des mises à jours..', true)
            }
        })
    }
}
